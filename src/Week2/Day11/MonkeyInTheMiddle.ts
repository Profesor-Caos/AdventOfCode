...
fs.readFile('./src/Week2/Day11/input.txt', 'utf8', (err: any, data: any) => {
    if (err) 
      throw err;
    else
    {
...
function partOne(text: string): number {
    let monkeyStrings: string[] = text.includes('\r\n') ? text.split('\r\n\r\n') :text.split('\n\n')
    let monkeys: Monkey[] = monkeyStrings.map(x => new Monkey(x));
    for (let i = 0; i < 20; i++) {
...
function getNLargest(counts: number[], n: number): number[] {
    let largestSoFar: number[] = [];
...
function getLowestIndex(nums: number[]): number {
