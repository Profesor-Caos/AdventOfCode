...
fs_1.default.readFile('./src/Week3/Day15/input.txt', 'utf8', (err, data) => {
    if (err)
        throw err;
    else {
...
function partOne(text, row) {
    let lines = text.includes('\r\n') ? text.split('\r\n') : text.split('\n');
    let sensors = lines.map(x => new Sensor_1.Sensor(x));
    let rowRanges = [];
...
function testNTimes(sampleSize, func, data) {
    let time = new Date().getTime();
...
function generateNRanges(n, sorted) {
    let numbers = [];
    for (let i = 0; i < n; i++) {
